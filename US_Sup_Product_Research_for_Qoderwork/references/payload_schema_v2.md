...
| 内容类型 | 谁负责 | 为什么 |
|---------|--------|--------|
| 章节结构/标题/编号 | 渲染模板固定 | 保证每次都有基础 10 章，启用 xCrawl 时增加站外信号章节，标题一致 |
...
| `chapters` | 不存在 | **核心新增**，10-11 章结构化数据+洞察；启用 xCrawl 时必须包含 `ch07_web_signal_intelligence` |
...
固定基础 10 个 key，每个对应一章。LLM 填写每章的结构化数据字段和 `insight` / `findings` / `conclusions` 自由文本字段。

启用 xCrawl 站外网络信号时，额外包含 `ch07_web_signal_intelligence`。该章节插入在 VOC 痛点之后、机会空白之前。
...
### ch07_web_signal_intelligence（可选；启用 xCrawl 时必填）
  ...
  "platform_summary": [
    {
      ...
      "consumer_language": ["noticed better endurance", "too expensive"]
    }
  ],
  "demand_signals": [
    {
      "signal": "社区讨论集中在线粒体健康和耐力恢复",
      "source_url": "https://example.com",
      "relevance": "验证功效方向中能量/代谢与运动恢复需求"
    }
  ],
  "ingredient_feedback": [
    {
      "type": "benefit",
      "feedback": "用户更常把该成分理解为长期健康老化支持，而不是即时体感",
      "sentiment": "neutral-positive",
      "product_implication": "Listing 叙事应偏长期健康和功能维持",
      "source": "Reddit / brand FAQ"
    }
  ],
  "safety_signals": [
    {
      "concern": "长期服用安全性和剂量疑问",
      "frequency": "medium",
      "source_platforms": ["Google", "Reddit"],
      "relevance": "需交给合规 Skill 检查安全表达和证据引用"
    }
  ],
  "brand_narratives": [
    {
      ...
      "implication": "新品牌应避开硬碰临床背书，从透明配方和价格切入"
    }
  ],
  "trend_signals": [],
  "insight": "站外信号的综合判断..."
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `coverage` | object | ✅ | 搜索/抓取覆盖情况 |
| `platform_summary` | array | ✅ | 平台级讨论与情绪汇总 |
| `demand_signals` | array | 推荐 | 站外需求验证信号 |
| `ingredient_feedback` | array | 推荐 | 成分效果、无效、体验、价格等反馈 |
| `safety_signals` | array | 推荐 | 风险或安全顾虑，只作风险信号 |
| `brand_narratives` | array | 推荐 | 竞品官网/品牌叙事 |
| `insight` | string | ✅ | 站外信号综合洞察 |
...
2. `chapters` 包含全部基础 10 个 key（ch01 ~ ch10）；启用 xCrawl 时包含 `ch07_web_signal_intelligence`
...
8. `excel_sheets` 包含 11 个基础必选 Sheet；启用 xCrawl 时额外包含 3 个 `Web情报_*` Sheet
