...
"""
US_Sup_Product_Research 交付生成/校验脚本 v2
...
用法：
  ...
  # 或使用相对路径（相对于脚本所在目录）：
  cd /path/to/skills/US_Sup_Product_Research/scripts
  python render_deliverables.py all --input ../payload.json
"""
...
CHAPTER_TITLES = {
    ...
    "ch07_web_signal_intelligence": "七、站外网络信号与成分反馈",
    "ch07_opportunity_gaps": "八、机会空白汇总",
    "ch08_strategic_recommendations": "九、品牌战略建议",
    "ch09_barriers_go_nogo": "十、进入壁垒与 Go/No-Go 评分",
    "ch10_appendix": "十一、数据来源与附录",
}
...
def _has_web_signal(payload: dict[str, Any]) -> bool:
    chapters = payload.get("chapters", {})
    data_sources = " ".join(str(item).lower() for item in payload.get("metadata", {}).get("data_sources", []))
    return "ch07_web_signal_intelligence" in chapters or "xcrawl" in data_sources or "web_signal" in data_sources


def _active_chapter_keys(chapters: dict[str, Any]) -> list[str]:
    if "ch07_web_signal_intelligence" in chapters:
        return CHAPTER_KEYS
    return [key for key in CHAPTER_KEYS if key != "ch07_web_signal_intelligence"]


def _required_sheets(payload: dict[str, Any]) -> list[str]:
    sheets = list(REQUIRED_SHEETS)
    if _has_web_signal(payload):
        sheets.extend(WEB_SIGNAL_SHEETS)
    return sheets
...
def build_output_dir(metadata: dict[str, Any], root: Path) -> Path:
    style = metadata.get("output_dir_style", "ingredient_date")
    if style == "brand_market_research":
        return root / "工作成果" / "brands" / metadata["brand"] / "市场调研" / metadata["category"] / metadata["version"]

    ingredient = metadata.get("ingredient") or metadata.get("keyword") or metadata.get("category")
    return root / "工作成果" / f"{_path_token(ingredient)}_{metadata['date']}"


def build_base_name(metadata: dict[str, Any]) -> str:
    keyword = metadata.get("keyword_slug") or _path_token(metadata["keyword"])
    return f'{metadata["date"]}_{metadata["site"]}_{keyword}'
...
def render_markdown(chapters: dict[str, Any], metadata: dict[str, Any]) -> str:
    ...
    for ch_key in _active_chapter_keys(chapters):
...
def _render_ch07_web_signal_md(ch: dict[str, Any]) -> str:
    lines: list[str] = []

    coverage = ch.get("coverage", {})
    if coverage:
        lines.append("### 采集覆盖\n")
        headers = ["搜索词数", "抓取页面数", "覆盖平台", "数据日期"]
        lines.append(_md_table(headers, [[
            _safe(coverage.get("search_query_count")),
            _safe(coverage.get("scraped_page_count")),
            _safe(", ".join(coverage.get("platforms", [])) if isinstance(coverage.get("platforms"), list) else coverage.get("platforms")),
            _safe(coverage.get("data_date")),
        ]]))

    platform_summary = ch.get("platform_summary", [])
    if platform_summary:
        lines.append("\n### 平台情绪与讨论主题\n")
        headers = ["平台", "样本数", "正面", "中性", "负面", "高频话题", "消费者语言"]
        rows = []
        for item in platform_summary:
            sentiment = item.get("sentiment_distribution", {})
            rows.append([
                _safe(item.get("platform")),
                _safe(item.get("mention_count")),
                _safe(sentiment.get("positive")),
                _safe(sentiment.get("neutral")),
                _safe(sentiment.get("negative")),
                _safe(", ".join(item.get("top_topics", []))),
                _safe(" / ".join(item.get("consumer_language", [])[:3])),
            ])
        lines.append(_md_table(headers, rows))

    demand_signals = ch.get("demand_signals", [])
    if demand_signals:
        lines.append("\n### 需求信号\n")
        for signal in demand_signals:
            source = signal.get("source_url") or signal.get("source")
            lines.append(f"- **{_safe(signal.get('signal'))}**：{_safe(signal.get('relevance'))}（来源：{_safe(source)}）")

    ingredient_feedback = ch.get("ingredient_feedback", [])
    if ingredient_feedback:
        lines.append("\n### 成分真实反馈\n")
        headers = ["反馈类型", "具体反馈", "情绪", "产品启示", "来源"]
        rows = [
            [_safe(item.get("type")), _safe(item.get("feedback")), _safe(item.get("sentiment")),
             _safe(item.get("product_implication")), _safe(item.get("source"))]
            for item in ingredient_feedback
        ]
        lines.append(_md_table(headers, rows))

    safety_signals = ch.get("safety_signals", [])
    if safety_signals:
        lines.append("\n### 风险/安全信号\n")
        headers = ["顾虑", "频率", "来源平台", "对选品的含义"]
        rows = [
            [_safe(item.get("concern")), _safe(item.get("frequency")),
             _safe(", ".join(item.get("source_platforms", []))), _safe(item.get("relevance"))]
            for item in safety_signals
        ]
        lines.append(_md_table(headers, rows))

    brand_narratives = ch.get("brand_narratives", [])
    if brand_narratives:
        lines.append("\n### 竞品站外叙事\n")
        headers = ["品牌/网站", "定位叙事", "来源", "对我方含义"]
        rows = [
            [_safe(item.get("brand")), _safe(item.get("positioning")),
             _safe(item.get("source")), _safe(item.get("implication"))]
            for item in brand_narratives
        ]
        lines.append(_md_table(headers, rows))
...
_MD_RENDERERS = {
    ...
    "ch07_web_signal_intelligence": _render_ch07_web_signal_md,
...
def render_html_report(chapters: dict[str, Any], metadata: dict[str, Any]) -> str:
    ...
    chweb = chapters.get("ch07_web_signal_intelligence", {})
    ...
    web_signal_html = ""
    if chweb:
        coverage = chweb.get("coverage", {})
        platforms = coverage.get("platforms", [])
        if isinstance(platforms, list):
            platforms = "、".join(platforms)
        web_signal_html += (
            f'<div class="web-summary">'
            f'<div><span>搜索词</span><strong>{_h(coverage.get("search_query_count", 0))}</strong></div>'
            f'<div><span>抓取页</span><strong>{_h(coverage.get("scraped_page_count", 0))}</strong></div>'
            f'<div><span>平台</span><strong>{_h(platforms)}</strong></div>'
            f'</div>'
        )
        web_signal_html += '<div class="web-grid">'
        for item in chweb.get("platform_summary", []):
            sentiment = item.get("sentiment_distribution", {})
            topics = ", ".join(item.get("top_topics", [])[:4])
            web_signal_html += (
                f'<div class="web-card">'
                f'<h4>{_h(item.get("platform"))}</h4>'
                f'<p>样本：{_h(item.get("mention_count"))} · 正/中/负：'
                f'{_h(sentiment.get("positive", 0))}/{_h(sentiment.get("neutral", 0))}/{_h(sentiment.get("negative", 0))}</p>'
                f'<p>{_h(topics)}</p>'
                f'</div>'
            )
        web_signal_html += '</div>'
        signals = chweb.get("demand_signals", [])[:4]
        if signals:
            web_signal_html += '<h3>站外需求信号</h3><ul class="signal-list">'
            for signal in signals:
                web_signal_html += f'<li><strong>{_h(signal.get("signal"))}</strong><br><span>{_h(signal.get("relevance"))}</span></li>'
            web_signal_html += '</ul>'
        if chweb.get("insight"):
            web_signal_html += f'<p class="insight">{_h(chweb.get("insight"))}</p>'
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .kpi-card .value {{ font-size: 18px; font-weight: 700; color: var(--gray-900); overflow-wrap: break-word; word-break: break-all; line-height: 1.3; }}
    ...
    .web-summary {{ display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-bottom: 16px; }}
    .web-summary div, .web-card {{
      background: white; border: 1px solid var(--gray-200); border-radius: 10px; padding: 14px 16px;
    }}
    .web-summary span {{ display:block; font-size: 12px; color: var(--gray-500); }}
    .web-summary strong {{ display:block; color: var(--gray-900); font-size: 20px; }}
    .web-grid {{ display:grid; grid-template-columns: repeat(3, 1fr); gap: 12px; }}
    .web-card h4 {{ margin: 0 0 8px; color: var(--gray-900); }}
    .signal-list {{ background: white; border: 1px solid var(--gray-200); border-radius: 10px; padding: 14px 18px 14px 32px; }}
    .signal-list li {{ margin: 8px 0; }}
    .signal-list span {{ color: var(--gray-500); }}
    ...
    @media (max-width: 768px) {{
      ...
      .pain-grid, .tier-grid, .web-grid, .web-summary {{ grid-template-columns: 1fr; }}
    }}
  </style>
</head>
<body>
...
def _parse_metric_from_text(text: Any, label_patterns: list[str]) -> float:
    """Extract a number after labels such as '月搜', 'CPC', or '竞品'."""
    raw = str(text or "")
    for label in label_patterns:
        match = re.search(rf"{label}\s*[:：]?\s*\$?\s*([\d,]+(?:\.\d+)?)", raw, re.IGNORECASE)
        if match:
            return float(match.group(1).replace(",", ""))
    return 0.0


def _product_sample_rows(excel_sheets: dict[str, Any]) -> list[dict[str, Any]]:
    """Prefer keyword organic sample rows; fall back to the legacy Top100 sheet."""
    rows = excel_sheets.get("关键词自然位产品_明细")
    if isinstance(rows, list) and rows:
        return rows
    rows = excel_sheets.get("类目销量Top100_明细")
    return rows if isinstance(rows, list) else []


def _row_lookup_by_asin(rows: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
...
def _build_dashboard_view_model(payload: dict[str, Any], chapters: dict[str, Any], metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    sample_rows = _product_sample_rows(excel_sheets)
    sample_by_asin = _row_lookup_by_asin(sample_rows)
    brand_sales_from_rows: dict[str, float] = {}
    for row in sample_rows:
        brand = str(row.get("品牌") or row.get("brand") or "").strip()
        if brand:
            sales = _extract_number(str(row.get("月销量", row.get("monthly_sales", 0))))
            brand_sales_from_rows[brand] = brand_sales_from_rows.get(brand, 0.0) + sales
    ...
    for b in ch05.get("brand_landscape", []):
        ...
        monthly_sales = _extract_number(str(b.get("monthly_sales", "")))
        if not monthly_sales and brand_name in brand_sales_from_rows:
            monthly_sales = brand_sales_from_rows[brand_name]
        brands.append({
            "label": brand_name,
    ...
    for k in ch02.get("keyword_comparison", []):
        change_text = k.get("search_volume_change", "")
        monthly_search = k.get("monthly_search")
        if monthly_search in (None, ""):
            monthly_search = _parse_metric_from_text(change_text, [r"月搜", r"月搜索量", r"monthly search"])
        cpc = k.get("cpc")
        if cpc in (None, ""):
            cpc = _parse_metric_from_text(change_text, [r"CPC"])
        competitors = k.get("competitors")
        if competitors in (None, ""):
            competitors = _parse_metric_from_text(change_text, [r"竞品", r"搜索结果竞品数", r"competitors"])
        keywords.append({
            "关键词": k.get("keyword", ""),
            "维度": k.get("segment", ""),
            "月搜索量": monthly_search or 0,
            "CPC": cpc or 0,
            "搜索结果竞品数": competitors or 0,
    ...
    for c in ch05.get("competitor_selection_logic", []):
        row = sample_by_asin.get(str(c.get("asin", "")).strip(), {})
        competitors.append({
            "brand": c.get("brand", ""),
            "asin": c.get("asin", ""),
            "type": c.get("type", ""),
            "price": c.get("price") or row.get("价格", 0),
            "sales": c.get("monthly_sales") or row.get("月销量", 0),
            "reviews": c.get("reviews") or row.get("评论数", 0),
    ...
    web_signals = {
        "coverage": chweb.get("coverage", {}),
        "platformSummary": chweb.get("platform_summary", []),
        "demandSignals": chweb.get("demand_signals", []),
        "safetySignals": chweb.get("safety_signals", []),
        "ingredientFeedback": chweb.get("ingredient_feedback", []),
        "brandNarratives": chweb.get("brand_narratives", []),
    }
...
def validate_xlsx(xlsx_path: Path, expected_sheets: dict[str, Any], required_sheets: list[str]) -> list[str]:
    errors = []
...
def validate_v2_chapters(chapters: dict[str, Any]) -> list[str]:
    ...
    errors = []
    for key in _active_chapter_keys(chapters):
    ...
    if "ch07_web_signal_intelligence" in chapters:
        chweb = chapters.get("ch07_web_signal_intelligence", {})
        if not chweb.get("platform_summary"):
            errors.append("ch07_web_signal_intelligence 缺少 platform_summary")
        if not chweb.get("insight"):
            errors.append("ch07_web_signal_intelligence 缺少 insight")
...
def validate_payload_structure(payload: dict[str, Any]) -> list[str]:
    ...
    for sheet in _required_sheets(payload):
        if sheet not in sheets:
            errors.append(f"payload.excel_sheets 缺少必选 Sheet: {sheet}")
    if _has_web_signal(payload) and "ch07_web_signal_intelligence" not in payload.get("chapters", {}):
        errors.append("metadata.data_sources 含 xCrawl/Web Signal，但 chapters 缺少 ch07_web_signal_intelligence")
    return errors


def validate_paths(metadata: dict[str, Any], paths: dict[str, Path]) -> list[str]:
    errors = []
    expected_dir = build_output_dir(metadata, Path("."))
...
def validate(payload: dict[str, Any], root: Path) -> tuple[dict[str, Path], list[str]]:
    ...
    if not errors:
        ...
        errors.extend(validate_xlsx(paths["xlsx"], payload["excel_sheets"], _required_sheets(payload)))
