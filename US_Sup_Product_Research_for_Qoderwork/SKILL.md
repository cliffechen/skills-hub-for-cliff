---
name: US_Sup_Product_Research_for_Qoderwork
description: Amazon US 膳食补充剂选品分析套件（Sorftime MCP + xCrawl MCP）。扫描类目市场、多维度关键词对比、Top100 九维度属性标注、交叉分析、竞品差评归类、站外信号采集、Go/No-Go 评分，输出标准化四件套（MD报告 + HTML精简 + Dashboard看板 + Excel数据）。
argument-hint: "[成分/产品关键词]"
user-invocable: true
---

# 选品分析器（US Supplement Product Research - Sorftime MCP）
...
- `scripts/render_deliverables.py`
  ...
  - 命令：
    - `python <SKILL_ROOT>/scripts/render_deliverables.py generate --input <payload.json> --root <PROJECT_ROOT>`
    - `python <SKILL_ROOT>/scripts/render_deliverables.py validate --input <payload.json> --root <PROJECT_ROOT>`
    - `python <SKILL_ROOT>/scripts/render_deliverables.py all --input <payload.json> --root <PROJECT_ROOT>`
...
- `scripts/parse_supplement_title.py`
  - 用途：保健品（膳食补充剂）标题属性解析脚本，从标题+五点描述中提取成分大类、剂型、剂量标注、功效方向、目标人群、包装规格、认证标签、配方类型
  - 适用场景：场景 5（膳食补充剂定向分析）的 Step 1.5 属性标注
  - 命令：`python scripts/parse_supplement_title.py --input top100_raw.json --output-dir ./`
  - 可选参数：`--details details.json`（product_detail 补充数据）
- `scripts/parse_amazon_search_xcrawl.py`
  - 用途：把 xCrawl 抓回的 Amazon 搜索结果 HTML 解析为产品卡片（cards）与自然位 ASIN（organic-ASIN）JSON
  - 适用场景：xCrawl 站外信号流程中，已用 xCrawl scrape 拿到搜索页原始 HTML 后做结构化抽取
  - 命令：`python scripts/parse_amazon_search_xcrawl.py --input <amazon_search_page1_raw.html> --keyword "<关键词>" --output-dir ./`
- `scripts/build_web_signal_payload.py`
  - 用途：把已采集的 xCrawl/搜索/抓取摘要（`web_signal_analysis.json`）归一化为 `chapters.ch07_web_signal_intelligence` 与 `Web情报_*` Excel sheets，供 `render_deliverables.py` 直接使用
  - 注意：本脚本不直接调用 xCrawl，仅做结构化组装；适用于 Step 1.8 站外网络信号采集完成之后

## References

- `references/payload_schema_v2.md`
  - 用途：v2 payload 结构定义（10-11 章 chapters + excel_sheets），准备 `render_deliverables.py` 输入数据包时查看
- `references/html_report_spec.md`
  - 用途：HTML 精简报告区块定义参考
- `references/analysis_patterns.md`
  - 用途：四种分析模式的模板与示例，报告写作时必须引用（至少使用 3 种）
- `references/supplement_dimensions.md`
  - 用途：膳食补充剂分析维度定义（9 个维度的分类值、解析规则、交叉分析建议），场景 5 专用
- `references/web_signal_sources.md`
  - 用途：xCrawl 站外网络信号采集策略，定义搜索词、来源优先级、去重规则与风险标注规则
- `references/web_signal_schema.md`
  - 用途：`web_signal_analysis.json` 与 `chapters.ch07_web_signal_intelligence` 的结构定义
...
- `agents/insight-writer.md`（v2 新增，替代 report-writer.md）
  - 用途：负责在固定 10-11 章结构中撰写分析洞察段落（不写表格格式/HTML）
- `agents/web-signal-analyst.md`
  - 用途：负责 xCrawl 站外信号采集与整理，输出消费者讨论、情绪、成分反馈、竞品叙事和风险信号
...
11. ⛔ **交叉分析必须解释原因**：每个"空白"/"薄供给"标签必须附带原因分析（技术限制？需求不存在？被市场忽视？产品实现难度？），不得只标注状态
...
xCrawl 用于补充 Sorftime 无法覆盖的站外讨论与实时网络信号。它不替代 Amazon 站内市场数据，只用于回答：消费者在 Google、Reddit、论坛、竞品官网、媒体页面和社媒页面中如何讨论该成分。

| 工具 | 用途 | 使用时机 |
|------|------|----------|
| `xcrawl_search` | 按关键词搜索美国英文网页结果 | 发现成分讨论、竞品官网、论坛帖子、媒体文章 |
| `xcrawl_scrape` | 抓取单页，输出 markdown / links / summary / json / screenshot | 抓取高价值页面并结构化提取消费者反馈 |
| `xcrawl_map` | 获取站点 URL 列表 | 竞品官网、品牌教育页、FAQ 页面发现 |
| `xcrawl_crawl` | 按规则批量抓取站点 | 少量竞品官网或专题内容抓取 |
| `xcrawl_key_status` | 查询 API Key 与积分余额 | 任务开始前检查 |

**安全要求**：
- 不得把 `XCRAWL_API_KEY` 写入可提交文件；项目中只使用 `${XCRAWL_API_KEY}` 占位
- xCrawl 抓到的安全、副作用、医学类讨论只作为“风险信号”，不作为合规或医学结论
- 合规仍由专用 FDA/FTC/专利/商标 Skill 处理

---

## QoderWork 运行环境适配

> 本节定义 QoderWork 平台上的运行时约定。Claude Code / Cursor / Kiro 用户可跳过。

### MCP 工具命名映射

QoderWork 的 MCP 工具使用完全限定名（`mcp__{ServerName}__{toolName}`），与本文档中简写名的对应关系：

| 文档内简写 | QoderWork MCP 全限定名 |
|------------|----------------------|
| **Sorftime 工具** | |
| `category_report` | `mcp__SorftimeMCP__category_report` |
| `category_search_from_product_name` | `mcp__SorftimeMCP__category_search_from_product_name` |
| `keyword_detail` | `mcp__SorftimeMCP__keyword_detail` |
| `keyword_search_results` | `mcp__SorftimeMCP__keyword_search_results` |
| `product_detail` | `mcp__SorftimeMCP__product_detail` |
| `product_reviews` | `mcp__SorftimeMCP__product_reviews` |
| `product_search` | `mcp__SorftimeMCP__product_search` |
| `category_trend` | `mcp__SorftimeMCP__category_trend` |
| `keyword_extends` | `mcp__SorftimeMCP__keyword_extends` |
| `potential_product` | `mcp__SorftimeMCP__potential_product` |
| 其他 Sorftime 工具 | `mcp__SorftimeMCP__{工具名}`（规则同上） |
| **xCrawl 工具** | |
| `xcrawl_search` | `mcp__xcrawl__xcrawl_search` |
| `xcrawl_scrape` | `mcp__xcrawl__xcrawl_scrape` |
| `xcrawl_map` | `mcp__xcrawl__xcrawl_map` |
| `xcrawl_crawl` | `mcp__xcrawl__xcrawl_crawl` |
| `xcrawl_key_status` | `mcp__xcrawl__xcrawl_key_status` |

**调用规则**：在 QoderWork 中，先用 `qw_mcp_get` 查询工具 schema，再用 `qw_mcp_call` 执行调用。
...
```
SKILL_ROOT = C:\Users\Nexus\.cc-switch\skills\US_Sup_Product_Research
PROJECT_ROOT = （用户选择的工作文件夹，即 QoderWork 中"已选择的文件夹"路径）
```

- **脚本调用**：`python "%SKILL_ROOT%\scripts\render_deliverables.py" all --input <payload.json> --root "%PROJECT_ROOT%"`
- **引用文档**：`references/`、`agents/`、`assets/` 等相对于 `SKILL_ROOT` 解析
- **输出目录**：`工作成果/` 目录创建在 `PROJECT_ROOT` 下（即用户的工作文件夹中）
- **中间数据**：`top100_raw.json`、`cross_analysis.json` 等中间文件存放在输出目录内

### 文件交付行为

QoderWork 中完成四件套渲染后：
1. 所有交付文件保存到 `PROJECT_ROOT/工作成果/{成分名}_{YYYYMMDD}/`
2. 使用 `present_files` 工具向用户展示最终交付文件（MD、HTML、Dashboard、XLSX）
3. 同时保存 `unified_payload.json` 供后续重新渲染
...
3. 约束条件（可选）：
   ...
   - 品类偏好：如 家居/电子/宠物
   - 预算：如 10万人民币
4. 已有优势（可选）：如有特定品牌、配方、渠道或内容优势
```

**如果用户未提供信息**：默认按「新手入门」场景，美国站，价格 $10-40，无特定已有优势。
...
**检查点**：新品分析完成且新品友好度评级已给出，再进入 Step 1.8。

### Step 1.8: xCrawl 站外网络信号采集（Web Signal Intelligence）

> 本步骤用于解决信息散落在 Google、Reddit、论坛、竞品官网、健康媒体、社媒页面等位置的问题。它补充“消费者怎么讨论这个成分”，不替代 Sorftime 的 Amazon 站内数据。

**触发条件**：
- 默认在场景 5 中执行；如果 xCrawl MCP 不可用，必须在报告中注明“站外信号未采集”
- 用户明确提到“网络讨论”“Reddit”“社媒情绪”“竞品官网”“成分反馈”“实时网络”时必须执行

**1.8.1 搜索发现**

使用 `xcrawl_search`，美国英文结果，默认每个 query 取前 5 条：

| 目的 | Query 模板 |
|------|------------|
| 泛口碑 | `{成分名} supplement review` |
| 功效反馈 | `{成分名} benefits` |
| 负面反馈 | `{成分名} side effects` |
| 社区讨论 | `{成分名} reddit supplement` |
| 购买疑虑 | `is {成分名} worth it` |
| 竞品叙事 | `{竞品品牌} {成分名}` |

**1.8.2 深度抓取**

对高价值 URL 使用 `xcrawl_scrape`：

- 输出格式：`markdown + links + summary + json`
- 需要看页面渲染时追加 `screenshot`
- 优先抓取 Reddit/论坛、竞品官网 FAQ、专业健康媒体、Amazon/Walmart 评论页、行业文章
- 单个域名默认最多 3 个页面，避免来源过度集中

**1.8.3 结构化分析**

输出 `web_signal_analysis.json`，至少包含：

- `coverage`：搜索词数、抓取页面数、覆盖平台、数据日期
- `platform_summary`：平台、样本数、正/中/负情绪、话题、消费者原话
- `demand_signals`：站外需求信号及来源 URL
- `ingredient_feedback`：效果、无效、体验、副作用、价格、剂型等反馈
- `safety_signals`：安全/副作用/剂量/长期服用顾虑，固定标注“需另行评估”
- `brand_narratives`：竞品官网或品牌页面如何讲这个成分
...
**检查点**：确认至少 3 类来源被覆盖，且每条关键站外结论都能追溯到 URL 或来源平台，再进入 Step 2。
...
| 资金壁垒 | 首批备货 + 广告 + FBA/物流 | `category_report` + 价格/销量估算 |
| 技术壁垒 | 是否需要认证/专利/模具 | WebSearch + 品类知识 |
| 合规壁垒 | FDA/UL/CE/FCC 等认证 | WebSearch（见下方站点合规表） |
| 原料与检测壁垒 | 原料形态、COA、第三方检测、稳定性要求 | `product_detail` + 站外资料 |
...
- [ ] 若执行了 Step 2.4 进入壁垒评估，壁垒汇总表完整？（Review/资金/技术/合规/原料与检测/品牌 + 等级 + 成本 + 时间）
...
- [ ] 所有「空白」/「薄供给」标签都有原因解释（技术/需求/被忽视/产品实现）？
...
### 场景5：膳食补充剂定向分析（Amazon US Dietary Supplements）

> 适用于分析 Amazon US 保健品（膳食补充剂）大类市场。与场景 4 的通用定向分析相比，场景 5 预置了保健品专用的 9 个分析维度和标题解析规则，无需执行 Step 1.4 维度自发现。
>
> **⛔ 本场景不做任何合规性分析**（FDA/FTC/专利/商标），合规由专用 Skill 处理。

**适用触发词**：supplement、dietary supplement、保健品、膳食补充剂、vitamin、probiotic、omega、collagen 等保健品相关关键词 + US 站点。

**与场景 4 的区别**：

| 步骤 | 场景 4（通用定向） | 场景 5（膳食补充剂） |
|------|-------------------|---------------------|
| Step 1.1 类目扫描 | 用 `category_search_from_product_name` 定位 | 同场景 4，但搜索范围限定 Dietary Supplements 大类 |
| Step 1.2 关键词 | 至少 3 维度对比 | **⛔ 保健品关键词维度**：成分大词（vitamin d）、功效词（immune support）、剂型词（gummies vitamin）、人群词（women's multivitamin）（至少 3 维度） |
| Step 1.4 维度发现 | 用户未指定时必做 | **跳过**——维度已预置（见 `references/supplement_dimensions.md`），直接进入 Step 1.5 |
| Step 1.5 属性标注 | 按品类自定义维度 | **⛔ 使用保健品 9 维度**（成分大类、剂型、剂量标注、功效方向、目标人群、包装规格、认证标签、配方类型、价格带），使用 `scripts/parse_supplement_title.py` |
| Step 1.6 交叉分析 | 按需 | **⛔ 至少 3 对**：成分大类×剂型、成分大类×价格带、剂型×价格带（详见 `references/supplement_dimensions.md`） |
| Step 2.2 竞品选择 | 按细分段选 | **按成分大类+剂型+价格带选**——每个主要成分大类至少 1 个代表，覆盖 Gummies/Capsules/Softgels 主流剂型 |
| Step 2.4 进入壁垒 | 6 类壁垒 | **⛔ 合规壁垒一栏固定填写**「需另行评估——本 Skill 不覆盖 FDA/FTC/专利/商标合规分析，请使用专用合规 Skill」 |
| Step 4 差异化建议 | 产品矩阵（物理规格） | **产品矩阵（成分配方方向）**——Tier 规格表的维度改为：成分大类、核心成分、剂型、目标剂量段、目标人群、认证标签组合、配方类型 |

**保健品关键词维度对比表**（Step 1.2）：

| 维度 | 示例关键词 | 说明 |
|------|-----------|------|
| 成分大词 | vitamin d, probiotics, fish oil, collagen | 最大流量入口，按成分搜索 |
| 功效词 | immune support supplement, joint supplement, sleep aid | 按功效需求搜索 |
| 剂型词 | gummies vitamin, liquid vitamin, chewable probiotic | 按剂型偏好搜索 |
| 人群词 | women's multivitamin, kids probiotic, prenatal vitamin | 按目标人群搜索 |
| 品牌词 | nature made, garden of life, NOW supplements | 品牌流量占比参考 |

**保健品属性标注执行方法**（Step 1.5）：

1. **标题正则自动标注**（使用 `scripts/parse_supplement_title.py`）
   - 输入：`top100_raw.json`
   - 输出：`top100_parsed.json` + `uncertain_products.json`
   - 覆盖率预估：70-80%（保健品标题通常包含成分、剂型、数量等关键信息）

2. **⛔ `product_detail` 补充验证**（针对置信度「低」的产品）
   - 对 `uncertain_products.json` 中的产品调用 `product_detail`
   - 从五点描述（Bullet Points）中提取缺失的成分、剂型、功效信息
   - 每批最多 8 个并行调用
   - **重点提取**：五点描述中通常包含 Supplement Facts 的文字版摘要（成分+剂量）

3. **功效方向推测规则**
   - 优先从标题中的 claim 关键词提取（如 "Immune Support"、"Joint Health"）
   - 标题无 claim 时，从五点描述提取，标注为「⚠️ 推测」
   - 标题和五点均无 claim 时，标注为「通用/未明确」
   - **不从成分反推功效**——例如不因为含 Vitamin C 就自动标注「免疫支持」，除非标题/五点明确提及

**保健品 VOC 差评维度归类**（Step 2.2）：

保健品差评按以下维度归类（替代通用品类的物理属性维度）：

| 痛点维度 | 典型差评内容 |
|----------|-------------|
| 效果/功效 | "没有感觉到效果"、"吃了一个月没变化" |
| 味道/口感 | "太难吃了"、"有股怪味"、"Gummies 太甜/太硬" |
| 吞咽体验 | "胶囊太大吞不下"、"卡喉咙" |
| 消化反应 | "吃了胃不舒服"、"恶心"、"拉肚子" |
| 包装/数量 | "瓶子太大"、"数量不够"、"密封不好" |
| 气味 | "打开瓶子有鱼腥味"、"闻起来很奇怪" |
| 成分/剂量 | "剂量太低"、"成分表和描述不一致" |
| 价格/性价比 | "太贵了"、"不值这个价" |
| 通用质量 | "收到过期产品"、"瓶盖坏了"、"颜色不对" |

**保健品产品矩阵 Tier 模板**（Step 4.3）：
...
| 维度 | 规格 | 决策依据 |
|------|------|----------|
| 成分大类 | [如：草本/植物类] | [为什么选这个方向] |
| 核心成分方向 | [如：Ashwagandha + 复合配方] | [基于交叉分析空白] |
| 剂型 | [如：Gummies] | [基于剂型×价格带交叉分析] |
| 目标剂量段 | [有标注/无标注] | [竞品剂量标注情况] |
| 目标人群 | [如：通用 / Women] | [基于人群分布数据] |
| 认证标签组合 | [如：Non-GMO + Vegan + Gluten-Free] | [基于认证标签×价格带分析] |
| 配方类型 | [单一成分 / 复合配方] | [基于配方类型分布] |
| 包装规格 | [如：60ct] | [基于包装规格分布] |
...
⛔ **场景 5 禁止事项**：
- 不做 FDA/FTC 合规分析（壁垒评估中合规栏固定标注「需另行评估」）
- 不做专利/商标风险查验
- 不做 Supplement Facts 标签设计
- 不从成分反推功效（只从标题/五点的 claim 文本提取）
- 不给出具体成分剂量建议（只分析市场现有产品的剂量标注情况）
...
- **标注数据来源**：每个数据点标注来源工具（如 `mcp__SorftimeMCP__product_search`）
...
### 品类特定经验：膳食补充剂（Dietary Supplement）

| 维度 | 解析要点 | 正则/规则 |
|------|----------|-----------|
| 成分大类 | "Vitamin D3" 归入维生素类而非特殊功能类；"Mushroom" 系列（Reishi/Lion's Mane/Cordyceps）归入草本/植物类；"Collagen Peptides" 归入氨基酸/蛋白类而非美容类 | 关键词映射表匹配，见 `references/supplement_dimensions.md` |
| 剂型 | "Veggie Capsule" = Capsules（不是独立剂型）；"Liquid Softgel" = Softgels；"Gummy Vitamin" 中 "Gummy" 是剂型不是品牌名 | 正则 + 同义词归一化 |
| 剂量 vs 包装数量 | "5000 IU" 是剂量，"360 Softgels" 是包装数量——两者都含数字+单位，必须区分。剂量单位：mg/mcg/IU/CFU；包装单位：count/ct/capsules/softgels | 先匹配包装数量正则排除，再匹配剂量正则 |
| 功效方向 | "Support" 是保健品标题中最高频的功效指示词（如 "Immune Support"、"Joint Support"）；但 "Supports Healthy..." 在五点中更常见 | 匹配 `\w+\s+support` 模式 |
| 复合 vs 单一 | "with" 连接两个成分 = 复合配方（如 "Vitamin D3 with K2"）；"BioPerine" / "Black Pepper Extract" 是吸收增强剂，不算独立成分 | "with" 后跟成分名 → 复合；已知辅助成分列表排除 |
| 认证标签堆叠 | 保健品标题经常堆叠 3-5 个认证标签（"Non-GMO, Vegan, Gluten-Free, GMP, Third-Party Tested"），这些标签影响定价但不影响成分分类 | 全部提取，计数作为独立分析指标 |
| 品牌名干扰 | 部分品牌名含成分词（如 "Nature's Bounty"、"Garden of Life"），不要把品牌名中的 "Nature" 误判为成分 | 品牌名通常在标题最前面，用逗号或空格与产品名分隔 |
| "Billion CFU" 格式 | 益生菌剂量常写为 "50 Billion CFU" 或 "100B CFU"，"Billion" 是数量级不是单位 | 正则：`(\d+)\s*(billion\s*)?CFU` |
...
> ⛔ 场景 5 使用独立的输出目录结构，按「成分名_报告日期」命名。该目录不上传 GitHub（已加入 .gitignore）。
> ⛔ **无论 Go/No-Go 结论是什么（GO / CONDITIONAL GO / HOLD / NO-GO），都必须输出完整四件套**。HOLD 和 NO-GO 同样需要留档，供后续复盘和趋势跟踪。

```
工作成果/{成分名}_{YYYYMMDD}/
├── unified_payload.json                                    # v2 数据包（唯一事实来源）
├── top100_raw.json                                         # 原始 Top100 数据
├── top100_parsed.json                                      # 属性标注后数据
├── uncertain_products.json                                 # 待验证产品
├── cross_analysis.json                                     # 交叉分析结果
├── voc_analysis.json                                       # VOC 差评分析
├── {date}_US_{成分名}_市场调研报告_{version}.md             # 完整 MD 报告
├── {date}_US_{成分名}_市场调研_数据_{version}.xlsx          # Excel 多 Sheet 数据
├── {date}_US_{成分名}_精简报告_{version}.html               # HTML 精简报告
└── {date}_US_{成分名}_可视化看板_{version}.html             # Dashboard 看板
```

**命名规则**：
- `{成分名}`：成分英文名，用连字符连接（如 `SAM-e`、`Vitamin-D3`、`Omega-3`）
- `{YYYYMMDD}`：报告生成日期（如 `20260427`）
- `{version}`：版本号（如 `v1_20260427`）
- 示例：`工作成果/SAM-e_20260427/20260427_US_SAM-e_市场调研报告_v1_20260427.md`

**⛔ 交付硬性规则（场景 5 补充）**：
- 即使 Go/No-Go 结论为 HOLD 或 NO-GO，仍必须组装 `unified_payload.json` 并调用 `render_deliverables.py all` 生成完整四件套
- 不得以「市场不值得进入」为由跳过交付流程
- 不得仅在聊天中输出分析结论而不生成文件
...
| **进入壁垒评估** | ⛔ | Review / 资金 / 技术 / 合规 / 原料与检测 / 品牌六类壁垒，含等级、成本/时间、前置条件 | 表格 + 结论段落 |
...
**存储位置**：场景 5（膳食补充剂成分分析）统一为 `工作成果/{成分名}_{YYYYMMDD}/`。通用品牌市场调研如需沿用 `工作成果/brands/[品牌名]/市场调研/[品类slug]/[version]/`，必须在 payload metadata 中显式设置 `output_dir_style: "brand_market_research"`。不得再写入仓库根目录或旧的 `选品分析/[YYYYMM]` 结构。
...
| **Web情报_搜索结果** | 必选（Step 1.8） | xCrawl 搜索 query 与候选 URL | 查询词、排名、标题、URL、平台、来源类型 |
| **Web情报_抓取明细** | 必选（Step 1.8） | xCrawl 抓取页面明细 | URL、平台、标题、摘要、情绪、关键话题、来源查询词、内容类型 |
| **Web情报_综合洞察** | 必选（Step 1.8） | 站外讨论、需求信号、风险信号、品牌叙事汇总 | 类型、对象、内容、主题、来源 |
...
|---------|--------------|----------|----------|--------------|
| 类目销量Top100_明细 | mcp__SorftimeMCP__category_report | 类目销量 Top100 榜单（该类目实时销量前 100 产品） | amzSite=US, nodeId=xxx | 2026-02-12 |
| 关键词自然位产品_明细 | mcp__SorftimeMCP__keyword_search_results | 关键词「portable charger」搜索自然位前 50 产品 | amzSite=US, searchKeyword=portable charger | 2026-02-12 |
...
1. 默认按场景 5 规范目录写入 `工作成果/{keyword}_{date}/`；如 metadata 设置 `output_dir_style: "brand_market_research"`，则写入 `工作成果/brands/{brand}/市场调研/{category}/{version}/`
2. v2 生成（4 个交付文件）：
...
3. 校验：
   - v2: 四件套齐全 + chapters 基础 10 章 + 可选 xCrawl 章节完整性 + 各章必填字段校验
   - Markdown 含 YAML 头
   - HTML 含 10 个必选区块
...
执行 `python <SKILL_ROOT>/scripts/render_deliverables.py all --input <payload.json> --root <PROJECT_ROOT>` 后，必须同时满足：

- 命令返回 `validate_ok`
- 正式交付文件位于 `工作成果/{成分名}_{YYYYMMDD}/`
...
```bash
python <SKILL_ROOT>/scripts/render_deliverables.py all --input <SKILL_ROOT>/evals/files/sample_payload_minimal.json --root <PROJECT_ROOT>
```
