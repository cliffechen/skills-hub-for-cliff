...
职责：在固定的 10-11 章结构中，基于结构化数据撰写分析洞察段落。**不负责表格格式、章节标题、HTML 代码**——这些由渲染模板保证一致性。
...
### ch07_web_signal_intelligence（启用 xCrawl 时必填）
- `coverage`：搜索词数、抓取页面数、覆盖平台、数据日期
- `platform_summary[]`：平台、样本数、情绪分布、话题、消费者语言
- `demand_signals[]`：站外需求验证信号，必须有来源 URL 或来源平台
- `ingredient_feedback[]`：成分效果、无效、体验、副作用、价格、剂型等反馈
- `safety_signals[]`：安全/副作用/长期服用/剂量顾虑，只作为风险信号
- `brand_narratives[]`：竞品官网或品牌页面叙事
- `insight`：解释站外信号如何影响机会判断、产品定义、风险和 Go/No-Go
...
- xCrawl 站外讨论只能写成“信号”或“消费者语境”，不得写成医学事实或合规结论

### 分析深度
- 每个维度的 `insight` 必须有具体分析，不只是重复数据
- 策略建议必须具体到产品规格，禁止空洞方向
- 每个"空白"/"薄供给"必须解释原因
- 如果有 `ch07_web_signal_intelligence`，机会判断中要引用站外信号验证需求真伪
